...
with st.sidebar:
    ...
    model = st.selectbox(
        "Model",
        options=["gpt-4o-mini", "gpt-4o", "deepseek-chat", "gemma4:12b"],
        index=0,
    )
...
if prompt := st.chat_input("Type your message..."):
    ...
    with st.chat_message("assistant"):
        try:
            client = openai_client if model.startswith("gpt") else deepseek_client if model.startswith("deepseek") else ollama_client
